...
河心有几片荷叶（分别记为 Y_1…Y_m）和几个石墩（分别记为 S——1…S——n）。图示如下：
青蛙的站队和移动方法规则如下：
...
*/
// 最小青蛙->小青蛙->...->最大青蛙，每次只有最大的能先走，
// 若有m个荷叶，没有石墩，则最多有dp[0]=0+m+1个青蛙
// 每增加一个石墩，把这个石墩视作对岸dp[i]=accumulate(dp,0,n-1)+m+1
#include <iostream>
#include <vector>
#include <numeric>
using namespace std;
int main(){
    ...
    vector<int> dp(n+1,0);
    int sum=0;
    for (int i=1;i<=n;i++)dp[i]=accumulate(dp.begin(),dp.begin()+i,m+1);
    cout<<dp[n];
    return 0;
}