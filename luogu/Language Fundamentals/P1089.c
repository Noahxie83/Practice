...
因此津津制定了一个储蓄计划：每个月的月初，在得到妈妈给的零花钱后，如果她预计到这个月的月
末手中还会有多于 100 元或恰好 100 元，她就会把整百的钱存在妈妈那里，剩余的钱留在自己手中。
例如 11月初津津手中还有 83 元，妈妈给了津津 300 元。津津预计11月的花销是 180 元，
那么她就会在妈妈那里存 200 元，自己留下 183 元。到了 11 月月末，津津手中会剩下 3 元钱。
...
#include <stdio.h>
int main(){
    int spend[12],saving[13]={0},cnt=0;
    int i=0,flag=0,x;
    for (i=0;i<12;i++){
        saving[i]+=300;
        scanf("%d",&spend[i]);
        if ((saving[i]-spend[i])/100>0){
            cnt+=(saving[i]-spend[i])/100;
            saving[i]=saving[i]-(saving[i]-spend[i])/100*100-spend[i];
        }
        else {
            saving[i]=saving[i]-spend[i];
        }
        if (saving[i]<0){
            flag=1;
            x=i+1;
